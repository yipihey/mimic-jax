...
- [What becomes comparable when SAGE16, SHARK, and Sapphire share one analysis language?](reports/three-model-response-foundation/index.md) — the pinned native Sapphire Pandya23 model joins the common registry through an isolated modern-JAX runtime and checksum-bound scientific artifact. The report validates its native adaptive trajectory, physical-coordinate Jacobians, finite differences, open-system budgets, fractional halo-input response, and local memory times beside SAGE16 and SHARK while keeping the different forcing and topology boundaries explicit.
- [Do SAGE16 and SHARK regulate the same matched disk in the same way?](reports/sage16-shark-response-foundation/index.md) — the first controlled use of the common SAM protocol: matched reservoirs and cooling supply, model-owned nonlinear rates, explicit conservation, Gyr-aligned local response, stable memory times, and direct validation against nonlinear evolution. It is deliberately not yet a same-tree population or closed-loop AGN comparison.
