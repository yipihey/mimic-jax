...
This fork is also developing an additive, JAX-native formalization of fiducial SAGE16 under [`mimic_jax/`](mimic_jax/). The current milestone covers the complete immutable SAGE16 state, every configured physical process, upstream tree inheritance, the complete live FoF schedule, and a legacy L-Halo tree/catalogue adapter. A real six-node Mini-Millennium branch matches all 42 z=0 catalogue fields; a 67-node branched/group tree matches 546 fields across all configured output snapshots at explicit tolerances. Conservation ledgers, physically normalized fractional responses, an explicit upstream-sequential timestep-refinement API, and [self-documenting run reports](docs/reporting.md) are also executable. Reports produce durable Markdown for GitHub/Obsidian, compact JSON for agents, referenced scientific arrays, comparison summaries, and an optional Quarto/GitHub Pages site without putting presentation logic into physics. Current equivalence remains selected-tree evidence, not yet a full-population or speedup claim. Start with [Mini-Millennium equivalence](docs/mini_millennium_equivalence.md), [SAGE16 state](docs/sage16_state.md), [tree inheritance](docs/tree_inheritance.md), [group evolution](docs/group_evolution.md), [reservoirs and transfers](docs/reservoirs_and_transfers.md), [numerical integration](docs/numerical_integration.md), [sensitivity](docs/sensitivity.md), and the gated [first scientific application program](docs/mimic_jax_scientific_program.md).
