...
This fork is also developing an additive, JAX-native formalization of fiducial SAGE16 under [`mimic_jax/`](mimic_jax/). The current milestone covers the complete immutable SAGE16 state plus upstream-verified reionization/infall, disk-radius and merger-clock setup, Type-1 satellite stripping, cooling/radio-mode, reincorporation, quiescent star formation/SN, disk instability, quasar mode, starburst feedback, delayed enrichment, and the ordered merger/disruption event phase, together with conservation ledgers, physically normalized fractional responses, and an explicit upstream-sequential timestep-refinement API. It does not yet claim complete tree or Mini-Millennium equivalence. Start with [SAGE16 state](docs/sage16_state.md), [reservoirs and transfers](docs/reservoirs_and_transfers.md), [disk radius and merger clock](docs/disk_radius_and_merger_clock.md), [mergers and disruption](docs/mergers_and_disruption.md), [numerical integration](docs/numerical_integration.md), [sensitivity](docs/sensitivity.md), and the gated [first scientific application program](docs/mimic_jax_scientific_program.md).
