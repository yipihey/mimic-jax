...
This fork is also developing an additive, JAX-native formalization of fiducial SAGE16 under [`mimic_jax/`](mimic_jax/). The current milestone covers the complete immutable SAGE16 state, upstream-verified tree inheritance, every configured physical process, and the complete live FoF-group module schedule, together with conservation ledgers, physically normalized fractional responses, and an explicit upstream-sequential timestep-refinement API. The remaining end-to-end boundary is the format-specific merger-tree/catalogue adapter; complete tree and Mini-Millennium equivalence are not yet claimed. Start with [SAGE16 state](docs/sage16_state.md), [tree inheritance](docs/tree_inheritance.md), [group evolution](docs/group_evolution.md), [reservoirs and transfers](docs/reservoirs_and_transfers.md), [mergers and disruption](docs/mergers_and_disruption.md), [numerical integration](docs/numerical_integration.md), [sensitivity](docs/sensitivity.md), and the gated [first scientific application program](docs/mimic_jax_scientific_program.md).
