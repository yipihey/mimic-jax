...
This fork is also developing additive, JAX-native formalizations of published semi-analytic models under [`mimic_jax/`](mimic_jax/). The current SAGE16 milestone covers the complete immutable state, every configured physical process, upstream tree inheritance, the complete live FoF schedule, and a legacy L-Halo tree/catalogue adapter. A real six-node Mini-Millennium branch matches all 42 z=0 catalogue fields; a 67-node branched/group tree matches 546 fields across all configured output snapshots at explicit tolerances. A complete 2,864-tree Mini-Millennium partition gives identical resolved stellar-mass-function bins, with small remaining field-level floating-point residuals kept visible in the validation record. Conservation ledgers, physically normalized fractional responses, separate exact-sequential and hybrid numerical formulations, Jacobian-aware adaptive integration of continuous flows, and [self-documenting run reports](docs/reporting.md) are executable. The [first science-program report](reports/mini-millennium-sage16-science-program/index.md) connects the familiar stellar mass function to parameter-response fingerprints, finite-epoch baryon-cycle responses, cooling versus AGN regulation, halo/stellar growth histories, population timestep refinement, and an adaptive convergence experiment that keeps finite maps outside Runge–Kutta stages. The complementary [galaxy-memory report](reports/sage16-linear-response/index.md) asks how long SAGE remembers gas-supply changes, which reservoirs set that response time, which fluctuation timescales reach star formation, and how stored AGN heating changes that response. The [differentiable-calibration report](reports/sage16-differentiable-calibration/index.md) fits a real SAGE stellar-mass-function observation, measures what one tangent calculation saves over finite differences, compares the workflow with familiar MCMC, and deliberately withholds final error bars after its first emulator fails held-out validation. The [minimal-model report](reports/sage16-minimal-model/index.md) tests how much of the z=0 stellar-mass prediction survives in four evolving states and keeps its failures for cold gas, SFR, fine-bin structure, and quenching explicit. The [SHARK Lagos23 report](reports/shark-continuous-foundation/index.md) pins and runs the upstream public CI population, exposes its exact 19-state disk/starburst flow plus an augmented BH/AGN memory state, implements finite constraints and hybrid merger/instability/stripping/BH events, validates complete controlled disk and starburst intervals against the native library, and exhaustively replays all 5,709,080 realized continuous population states through a batched JAX kernel. Three of 62,799,880 rates sit just outside the predeclared 0.011% strict gate but inside the explicit 0.015% BR06 quadrature warning band; the report keeps both this measured warning and the remaining topology-owning per-ID catalogue gate visible. The [SAGE16--SHARK interoperability audit](reports/sage16-shark-interoperability-audit/index.md) projects both native catalogues through one physical-unit/observable contract and measures tree portability separately from physics parity. The [first common-response experiment](reports/sage16-shark-response-foundation/index.md) adds a minimal `load_model` protocol, matches one local disk and cooling supply, converts both Jacobians to Gyr, and validates the local prediction against nonlinear evolution. [Sapphire Pandya23 is now a native third configured model](docs/sapphire_interoperability.md): mimic-jax delegates its pinned v0.130 equations and Diffrax solve to an isolated modern-JAX environment, imports a checksum-bound trajectory/rate/Jacobian artifact, and applies the same conservation, parameter-normalization, local-response, mode, and report concepts without copying Sapphire physics. The [three-model report](reports/three-model-response-foundation/index.md) validates native AD against finite differences, adaptive refinement, and open-system budgets, then compares qualified supply filtering and memory times while leaving same-history population equivalence explicitly not evaluated. The [model-comparison guide](docs/model_comparison.md), [SHARK guide](docs/shark_lagos23.md), and [Sapphire guide](docs/sapphire_interoperability.md) define those claim boundaries. Reports produce durable Markdown for GitHub/Obsidian, compact JSON for agents, referenced scientific arrays, pairwise or multi-model summaries, and an optional Quarto/GitHub Pages site without putting presentation logic into physics. No general JAX speedup is claimed yet. Start with [Mini-Millennium equivalence](docs/mini_millennium_equivalence.md), [SAGE16 state](docs/sage16_state.md), [tree inheritance](docs/tree_inheritance.md), [group evolution](docs/group_evolution.md), [reservoirs and transfers](docs/reservoirs_and_transfers.md), the [hybrid SAGE16 classification](docs/sage16_hybrid_system.md), [numerical integration](docs/numerical_integration.md), [sensitivity](docs/sensitivity.md), [differentiable calibration](docs/differentiable_calibration.md), and the [first scientific application program](docs/mimic_jax_scientific_program.md).
