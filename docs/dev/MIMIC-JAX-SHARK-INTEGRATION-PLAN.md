...
**Status:** Active — the pinned Lagos23 controlled process/event kernels, reference intervals, continuous counterpart, managed native population backend, exhaustive realized-population RHS replay, canonical catalogue/observable projection, and canonical tree audit are implemented. Independent topology-owning JAX population evolution, foreign-tree execution, and same-forcing comparison remain open.
**Date:** 2026-08-20
...
V0 and V1 are implemented. V2 covers controlled BR06 star-formation/angular-momentum, Lagos13 stellar-feedback, reincorporation, Sobacchi13 reionisation, Croton06 cooling, deterministic Lagos23 AGN/QSO branches, Griffin19 spin in the burst sequence, finite cooling/BH caps, and the event maps used by the pinned configuration. The C++ harness is linked to the pinned upstream library; it does not duplicate the equations in Python. Four BR06 cases agree within `5e-6` relative (the JAX implementation uses deterministic 128-node quadrature rather than upstream's configured 5%-tolerance GSL integral). Feedback and realized reincorporation cases are exact in float64, reionisation branches agree exactly, and the packaged 8 × 227 Cloudy table, cooling, and deterministic AGN/QSO cases agree at float64 precision. V3 passes for the controlled ordered disk interval and event-triggered starburst at explicitly recorded tolerances. V5 passes for the controlled continuous disk flow. The complete native CI run has also supplied 5,709,080 realized disk/starburst RHS states: 62,799,877 of 62,799,880 named rates pass the strict `rtol=1.1e-4` gate, all values pass the explicit `rtol=1.5e-4` BR06 warning envelope, and all 108,472,520 structural routing comparisons pass. This is exhaustive population-physics shadow evidence, not V4: native SHARK still owns topology, states, and event scheduling. V4 (independent per-ID JAX evolution of the complete CI tree) and V6 (common-forcing SAGE comparison) remain open.
...
| Stellar metallicity | `MetalsStellarMass/StellarMass` | `(mstars_metals_disk + mstars_metals_bulge)/(mstars_disk + mstars_bulge)` | Direct mass-weighted comparison; SAGE `MetalsBulgeMass` is already a subset and must not be added again |
...
### Phase 6 — Shared observables and SAGE comparison (catalogue contract implemented; same forcing open)

The `ComparisonCatalogue` projection, shared observable reductions, field provenance, observable-capability audit, and native-output comparison report are implemented. Eighteen of twenty-five reviewed observable definitions are available for both current catalogues; qualifications remain visible for cold-gas phases, reservoir ownership, metallicity interpretation, halo mass, velocity proxies, positions, and disk size. `CanonicalMergerTree` now projects representative L-Halo and VELOCIraptor trees and measures missing/derived fields separately from topology-driver readiness. Neither foreign-tree population path is yet runnable. The remaining phase runs both models on canonical common forcing and publishes paired predictions, differences, fractional responses, convergence/error bands, and provenance-rich comparison reports.
...
The next gate is not another isolated prescription. It is the independent topology-owning JAX population driver on the public CI merger tree: evolve all 20,174 trees, match stable galaxy IDs, report unmatched objects and threshold flips, and compare every shared observable. The exhaustive realized-population RHS replay is retained as a strong but separately labelled precursor. In parallel, foreign-tree adapters must resolve the explicit blockers in the [interoperability audit](../../reports/sage16-shark-interoperability-audit/index.md): progenitor/ownership semantics, concentration and size inputs, velocity dispersion/virial radius, and angular-momentum/spin conventions. Only then can the inter-model science gate run SAGE16 and SHARK on common halo forcing without mistaking tree/cosmology differences for baryonic-model differences.
