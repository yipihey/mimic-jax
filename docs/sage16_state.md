...
`HaloForcing` is separate because merger-tree properties are external forcing, not baryonic state. It includes every instantaneous field read by the SAGE16 processes plus the identity and phase-space fields required for faithful descendant inheritance: unique galaxy IDs, position, velocity, velocity dispersion, spin, `Vmax`, and most-bound-particle ID. `Sage16Parameters`, `Sage16Units`, `StepContext`, and `InheritanceDescendant` are also immutable PyTrees, so the numerical boundaries for physics and tree-state propagation remain explicit.
