"""Observed calibration products used by the SAGE16 examples."""
...
def load_baldry2008_stellar_mass_function(
    ...
    *,
    ...
    """Load the exact Baldry et al. table used by the SAGE plotting example.

    The conversions deliberately reproduce ``stellar_mass_function.py``:
    masses lose their published ``h`` dependence and receive the same
    Salpeter-to-Chabrier shift, while number densities are multiplied by
    ``h**3``.  The quoted widths are retained as an observational envelope;
    treating them as independent Gaussian errors is a separate, explicit
    likelihood choice made by the calibration application.
    """
